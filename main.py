...
# Вставить токен в переменные среды текущего пользователя или написать свой токен вмето получения
TOKEN = os.environ["T_BOT"]
...
def receivedSumValute(message):
    ...
    if(inValute and toValute):  # Если введены обе валюты для перевода
        ...
        if not message.text.isalpha():  # Если это число (float или int)
